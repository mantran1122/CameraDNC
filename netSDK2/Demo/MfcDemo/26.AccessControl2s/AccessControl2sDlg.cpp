...
BEGIN_MESSAGE_MAP(CAccessControlDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_PIC_SUBSCRIBE, &CAccessControlDlg::OnBnClickedButtonPicSubscribe)
END_MESSAGE_MAP()
...
void CAccessControlDlg::InitCtrls()
{
	...
	GetDlgItem(IDC_BUTTON_PIC_SUBSCRIBE)->EnableWindow(FALSE);
}
 	...
 	case UI_STATE_LOGIN:
 		{
 			...
 			GetDlgItem(IDC_BUTTON_LOGOUT)->EnableWindow(TRUE);
  			GetDlgItem(IDC_BUTTON_START2)->EnableWindow(TRUE);
  			GetDlgItem(IDC_BUTTON_STOP2)->EnableWindow(FALSE);
			GetDlgItem(IDC_BUTTON_PIC_SUBSCRIBE)->EnableWindow(TRUE);
			BYTE i= 0;
			int nChn = 4;
			int nAccessGroup = 0;
...
void CAccessControlDlg::OnBnClickedButtonPicSubscribe()
{
	// TODO: 在此添加控件通知处理程序代码
	CDialogPicEvent picEvent;

    // 获取当前选中项的索引
	int count = m_cbChannel.GetCount();
	picEvent.setChnCount(count);
