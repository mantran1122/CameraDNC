...
typedef enum _CMD_TYPE
{
	...
	PLAY_CMD_RecordEncryptParam = 10,//获取加密参数
} CMD_TYPE_E;

/* PLAY_CMD_GetRenderTime对应的查询结构体 */
...
typedef enum
{
	AV_SYNC_VIDEO_MAIN,		  // 以视频为基准（非音视频同步方式）
...
typedef struct
{
	...
	int				nTotalChannel;			//音频总通道数（适用于多路）
	int				nCurChannel;			//音频当前通道
	
	unsigned char	nEncryptIV[16];			//加密初始化向量
	unsigned char	nEncryptVKid[16];		//加密VKID

	int				nEncodeType;			//编码方式
	int				nStreamType;			//封装格式
	int				nStride;				//画面跨距，单位像素
	
	unsigned char	bThrowFrame;			//是否抽帧
	unsigned char	bReserved[3];			//对齐

	int				nReserved[42];			// 保留字段
}FRAME_INFO_EX;
...
typedef struct 
{
	...
	int				nType;						// 视频帧类型，T_AUDIO16，T_RGB32，T_IYUV
    unsigned int	nTimeStamp;		            // 时间戳
	int				nDecodeChannel;				// 拼接码流解码通道号,如果拼接码流有8个通道,依次为0~7
	int				nReserved[57];				// 保留字段
}FRAME_DECODE_INFO;
...
typedef enum
{
    ...
    RECORD_STATUS_CAMERA_OPEN_SUCCESS,					//相机打开成功
    RECORD_STATUS_CAMERA_RUNTIME_BAD_ERROR,				//相机运行时异常
    RECORD_STATUS_CAMERA_ENTER_BACKGROUND,				//程序进入后台（IOS才会提示）
    RECORD_STATUS_CAMERA_WILL_ENTER_FOREFROUND,			//程序进入前台（IOS才会提示）
    RECORD_STATUS_ENCORDER_CREATE_FAILED,				//编码器创建失败
    RECORD_STATUS_ENCORDER_FRAME_FAILED					//编码帧塞入失败
}CAMERA_RECORD_NOTIFY;

/**
 * 视频 采集的事件回调函数。
 *
 * @param[in] notifyType 事件类型
 * @param[in] extraData 预留的额外数据指针
 * @param[in] pUserData 用户数据
 */
...
typedef enum
{
    CAMERA_TYPE_BACK = 0,  	//后置镜头
    CAMERA_TYPE_FRONT		//前置镜头
}CAMERA_TYPE;
...
UINT cameraType;		//相机类型
UINT captureWidth;		//分辨率宽
UINT captureHeight;		//分辨率高
UINT frameRate;         //期望帧率
UINT outPutWidth;		//编码输出的宽【和采集不一致，会有性能消耗】
UINT outPutHeight;		//编码输出的高【和采集不一致，会有性能消耗】
BOOL isRotateFrame;		//是否对帧数据进行旋转【性能消耗】
...
UINT cameraType;		//相机类型
UINT width;  			//比例宽，ratio like: 9 : 16
UINT height;			//比例高
...
UINT count;				//数组个数
UINT * arrayPixelWidth;	//宽数组
UINT * arrayPixelHeight;//高数组

}CAMERA_RECORD_CAPABILITY;

/**
 * 获取相机的能力集，目前主要是分辨率支持信息
 *
 * @param[in] pData 能力集数据块指针
 * @return void
 */
 ...
 * 打开相机视频采集功能。
 *
 * @param[in] pProc 视频采集数据回调指针
 * @param[in] pNotify 相机、录制状态通知回调
 * @param[in] pParam 视频录制参数
 * @param[in] pUserData 回调UserData
 * @return BOOL，成功返回TURE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
...
typedef enum
{
	...
	ADAPTIVE_CACHE,			// 自适应缓冲, 根据网络数据波动进行自适应缓存调整, 缓存下降的速度较慢, 最小有2帧数据缓存
	REALTIME_FIRST,			// 零缓存优先, 来一帧解一帧, 不进行任何数据缓存, 适用于对延迟要求非常高的场景
	FLUENCY_FIRST,			// 流畅优先, 最小1秒数据缓存, 保证流畅性 
} CACHE_MODE;
 ...
 *
 * @param[in] nPort 通道号
 * @param[in] nStream 1 视频流(YUV格式) 2 音频流 3 复合流 4 视频流(BGR24格式)
 * @return BOOL，成功返回TRUE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
...
typedef enum
{
	...
	STREAM_TYPE_RAW_MJPEG = 161,	// MJPEG裸数据
}STREAM_TYPE;
...
typedef enum
{
	...
	STREAM_ERROR_FLAGS_DECODE_VERIFY,		/*编码数据校验错误*/
}STREAM_ERROR;
...
typedef struct 
{
	...
	unsigned char     nLevel;             //svac2.0码流等级，当前仅对svac2编码有意义
 ...
 *
 * @param[in] nPort 通道号
 * @param[in] szPlayToken playToken
 * @param[in] szPlayTokenKey  playToken秘钥
 * @param[in] szDeviceID 设备序列号
 * @param[in] szOutKey 原始秘钥
 * @param[in] nOutKeyLength  原始秘钥长度
 * @return BOOL，成功返回TRUE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetOriginalKey(LONG nPort, const char* szPlayToken, const char* szPlayTokenKey, const char* szDeviceID, unsigned char* szOutKey, int* nOutKeyLength);

//拉流信息
typedef struct
{
	char*  pSdpInfo;        //sdp信息 
	char*  pDevLoginName;	 //设备登录名
	char*  pDevLoginKey;	 //登录密码
	int	   nSsrc;		     //同步源			
	char nReserved[32];		 //保留字段    
}DESCRIBE_INFO; 

/**
 * 设置原始秘钥等信息，用于码流秘钥推导并解密。
 *
 * @param[in] nPort 通道号
 * @param[in] nDecryptType 码流加密类型
 * @param[in] nFrameType 帧类型（mikey解密区分视频和音频帧）
 * @param[in] szKey  原始秘钥
 * @param[in] stStreamInfo  sdp、设备登录名、登录密码、ssrc等信息
 * @return BOOL，成功返回TRUE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetWebSecurityKey(LONG nPort, int nDecryptType, int nFrameType, const char* szKey, DESCRIBE_INFO* stStreamInfo);			
...
typedef enum
{
	...
	ENCRYPT_SM4_OFB,                     /*国密加密算法SM4_OFB*/  //不知道具体是哪种国密加密，就填这种SP_ENCRYPT_SM4_OFB
}ENCRYPT_TYPE;
...
typedef struct
{
	...
	void* pDevHandle;			    //设备句柄，自研35114平台方案
	void* pAppHandle;			    //应用句柄，自研35114方案中文件加密下载导出使用
	void* pContainerHandle;			//容器句柄，自研35114平台方案
	char* pReserved[9];             //保留字段    
}DECRYPT_PARAM; 
...
typedef struct
{
	...
	unsigned char encodetype; //加密类型,详见ENCRYPT_TYPE
	char		  reserved[2];		
...
typedef enum
{
	...
	DATA_RECORD_RESIZE_ASF,			// 录制ASF(编解码) 
...
typedef enum _IVS_TYPE
{
	...
	IVSINFOTYPE_DATA_WITH_VIDEO_INFO_OVERLAY = 25,//视频信息遮挡帧（0x23）
...
typedef enum _STATISTIC_TYPE
{
	...
	VIDEO_DECODE_MEMORY_ERROR,		// 解码内存不足
}STATISTIC_TYPE;
